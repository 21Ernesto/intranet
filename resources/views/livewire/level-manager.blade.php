<div>
    <div x-data="{ open: false }">
        <div class="flex justify-between">
            <h1 class="text-2xl font-bold mb-4">Lista de niveles</h1>
            <div>
        ...
        <div x-show="open" class="fixed inset-0 overflow-y-auto flex items-center justify-center z-50">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="save" class="mb-4 mt-4">
                    <h1 class="pl-5 text-3xl font-semibold mb-4">{{ $editId ? 'Actualizar' : 'Crear' }} nivel</h1>
                    <div class="px-5 mb-3">
                    ...
                    @foreach ($levels as $level)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-end">
                                <button wire:click="edit({{ $level->id }})" @click="open = true"
                                    title="Editar {{ $level->name }}"
                                    class="text-indigo-600 hover:text-indigo-900 mr-2">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button type="button" onclick="confirmDeletion({{ $level->id }})"
                                    title="Eliminar {{ $level->name }}" class="text-red-600 hover:text-red-900 mr-4">
