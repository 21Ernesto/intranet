            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="save" class="mb-4 mt-4">
                    ...
                    <div class="px-5 mb-3">
                        ...
                        <input type="text" wire:model="description" id="description" placeholder="Descripción"
                            class="border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full">
                        @error('description')
                            <span class="text-red-400 text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-end">
                                ...
                                <button type="button" onclick="confirmDeletion({{ $category->id }})"
                                    title="Eliminar {{ $category->name }}" class="text-red-600 hover:text-red-900 mr-4">
