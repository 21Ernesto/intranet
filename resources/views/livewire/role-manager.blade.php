        ...
        <div x-show="open" class="fixed inset-0 overflow-y-auto flex items-center justify-center z-50">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="save" class="mb-4 mt-4">
                    <h1 class="pl-5 text-3xl font-semibold mb-4">Crear nuevo rol</h1>
                    <div class="px-5 mb-3">
                    ...
                    <div class="px-5 mb-3">
                        ...
                        <div class="flex items-center space-x-4">
                            <div>
                                <input type="radio" id="student" name="role_key" value="student" wire:model="key"
                                    class="form-radio h-4 w-4 text-indigo-600">
                                <label for="student" class="ml-1">Estudiante</label>
                            </div>
                            <div>
                                <input type="radio" id="teacher" name="role_key" value="teacher" wire:model="key"
                                    class="form-radio h-4 w-4 text-indigo-600">
                                <label for="teacher" class="ml-1">Docente</label>
                            </div>
                            <div>
                                <input type="radio" id="coordinator" name="role_key" value="coordinator"
                                    wire:model="key" class="form-radio h-4 w-4 text-indigo-600">
                                <label for="coordinator" class="ml-1">Coordinador</label>
                            </div>
                        </div>
                        @error('key')
                            <span class="text-red-400 text-xs">{{ $message }}</span>
                    ...
                    @foreach ($roles as $role)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-end">
                                <button wire:click="edit({{ $role->id }})" @click="open = true"
                                    title="Editar {{ $role->name }}"
                                    class="text-indigo-600 hover:text-indigo-900 mr-2">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button type="button" onclick="confirmDeletion({{ $role->id }})"
                                    title="Eliminar {{ $role->name }}" class="text-red-600 hover:text-red-900 mr-4">
